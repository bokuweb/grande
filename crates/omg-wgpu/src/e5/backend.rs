...
//! the head behind omg-core's [`Decider`], so `omg serve --model <e5
...
use omg_core::{Decider, Diagnostics, Distributions, Error, Mode, Request, Response};
...
impl E5Backend {
    ...
    /// `manifest.json` and a `config.json` with `omg_e5`.
...
impl Decider for E5Backend {
    ...
    fn answer(&mut self, req: &Request, _mode: Mode) -> omg_core::Result<(Response, Diagnostics)> {
    ...
    fn answer_many(
        ...
        reqs: &[&Request],
        _mode: Mode,
    ) -> Vec<omg_core::Result<(Response, Diagnostics)>> {
        let tok = Tok(&self.tokenizer);
        pollster::block_on(
            self.engine
