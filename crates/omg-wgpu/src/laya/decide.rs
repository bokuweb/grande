...
use omg_core::render::Kind;
use omg_core::{Diagnostics, Question, RenderedBranch, Request, Response, Usage};

use super::prompt::{self, Tokenize};
use super::{LayaConfig, LayaEngine, Output, Sequence};

/// A question's option keys in omg's original order, with the Laya
/// option index of each: identical for Choice / Score, swapped for Noul
/// (omg lists `true` first, Laya `false`).
...
/// order (omg original indices) where given; plus the state's token
/// count.
pub fn sequences(
    cfg: &LayaConfig,
    tok: &dyn Tokenize,
