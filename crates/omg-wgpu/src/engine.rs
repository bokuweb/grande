...
use half::f16;
use omg_core::{BranchOutput, BranchTokens, GroupOutput, PrefixSource, Want};
...
impl Kernels {
    ...
    pub(crate) fn get(&mut self, k: K, hd: usize, quant: u32) -> &Kernel {
        ...
        if !self.map.contains_key(&key) {
            ...
            let unchecked = std::env::var("OMG_WGPU_CHECKED").is_err();
            ...
            let module = if unchecked {
                ...
                // OMG_WGPU_CHECKED=1 to get the clamps back when chasing
...
/// anyway, so a restore is exact), per layer that owns a K/V buffer; a row
/// is all K heads then all V heads of one token. Sliding
...
impl SavedState {
    ...
    pub fn from_bytes(bytes: &[u8], model_id: &str) -> Result<Self> {
        ...
        if c.take(6)? != b"GRWS1\0" {
            bail!("not a omg wgpu state file");
        }
        let n = c.u32()?;
...
/// whether GPU timestamp profiling is on (OMG_WGPU_PROFILE, native only).
pub(crate) async fn open_device(
    ...
    #[cfg(not(target_arch = "wasm32"))]
    let profile = std::env::var("OMG_WGPU_PROFILE").is_ok()
...
impl Engine {
    ...
    async fn run<'s>(
        ...
        let mut plan: Vec<Dispatch<'s>> = Vec::with_capacity(self.param_slots);
        {
            ...
            #[cfg(not(target_arch = "wasm32"))]
            let n_layers = std::env::var("OMG_WGPU_LAYERS")
                .ok()
            ...
            // only a OMG_WGPU_LAYERS-truncated pass (whose last fused norm
        ...
        let mut enc = self
            .device
            .create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some("omg") });
        let pipeline_of =
    ...
    async fn save_state_at(&self, prefix: &[u32], row0: usize) -> Result<SavedState> {
        ...
        for (l, buf) in self.kv_bufs.iter().enumerate() {
            ...
            let bytes = ((r1 - r0) * 2 * cfg.kv_heads * cfg.head_dim[l] * 4) as u64;
        ...
        for &(l, r0, rows, off) in &chunks {
            let n = rows * 2 * cfg.kv_heads * cfg.head_dim[l];
    ...
    pub fn restore_state(&self, state: &SavedState) -> Result<()> {
        ...
        for (l, r0, bytes) in &state.layers {
            ...
            let row = 2 * cfg.kv_heads * cfg.head_dim[*l];
