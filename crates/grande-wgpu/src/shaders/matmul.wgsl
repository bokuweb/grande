...
// weight.wgsl, prepended; QUANT picks the decoder). A 32x128 output tile per
// workgroup of 128 invocations, each owning 4 consecutive rows x 8 consecutive
...
// hold (k, k+1) of one row / column, so a step of the inner loop is 3 vec4
// loads and 12 unpacks for 64 FMAs, and the tiles are half the bytes of f32
// ones. X is rounded to f16 on the way in (as llama.cpp's Metal mat-mul does).
// Measured on an M4 (E2B, 313 tokens) and rejected: 64-row tiles (fewer,
// longer workgroups: the small-N layers mm_o / mm_down lose 10-20%), an f32 X
// tile (+30%: the extra workgroup bandwidth costs more than the unpacks),
...
// workgroup stores. X: one k pair of one row quad. W (invocations 0..64
...
fn load_stage(li: u32, row0: u32, col0: u32, k0: u32) -> Stage {
    var s: Stage;
    // X: k pair li % 16 of row quad li / 16; a simdgroup reads two quads'
    // 128 contiguous bytes per row.
    let kp = li % KP;
    {
        let g = li / KP;
        var v = vec4<u32>(0u);
