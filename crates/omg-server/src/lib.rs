...
//! `X-Omg-Batch` header says how many shared its pass. Scale further by
...
use omg_core::readout::Distribution;
use omg_core::{Decider, Diagnostics, Error, Mode, RenderedBranch, Request};
use serde_json::{json, Value};
use tokio::sync::oneshot;

pub const ALIASES: [&str; 3] = ["grande-latest", "jev-latest", "jev-preview"];

type Answered = omg_core::Result<(omg_core::Response, Diagnostics)>;
type Distributed = omg_core::Result<(Vec<(RenderedBranch, Distribution)>, Diagnostics)>;
...
async fn models(
    ...
    let mut list: Vec<Value> = ALIASES
        ...
        .collect();
    list.push(
        json!({"name": s.model_id, "description": "local omg model", "release_date": "2026-09-19"}),
    );
    Ok(Json(json!({"models": list})))
}

fn diag_headers(diag: &omg_core::Diagnostics, model: &str, ms: u128) -> HeaderMap {
    ...
    put(&mut h, "x-omg-backend", model.to_string());
    put(
        &mut h,
        "x-omg-prefix-tokens",
    ...
    put(&mut h, "x-omg-passes", diag.passes.to_string());
    if !diag.baseline.is_empty() {
        put(&mut h, "x-omg-calibrated", "contextual".to_string());
    }
    if let Some(src) = diag.prefix_source {
        put(&mut h, "x-omg-state", src.as_str().to_string());
    }
    if diag.orders > 1 {
        put(&mut h, "x-omg-orders", diag.orders.to_string());
        if let Some(s) = diag.order_spread.values().cloned().reduce(f64::max) {
            put(&mut h, "x-omg-order-spread-max", format!("{s:.4}"));
        }
    }
    ...
    put(&mut h, "x-omg-batch", diag.batch.max(1).to_string());
    if !diag.rechecked.is_empty() {
        put(&mut h, "x-omg-rechecked", diag.rechecked.join(","));
    }
    put(&mut h, "x-omg-latency-ms", ms.to_string());
    if let Some(m) = diag.candidate_mass.values().cloned().reduce(f64::min) {
        put(&mut h, "x-omg-candidate-mass-min", format!("{m:.4}"));
    }
    h
}
...
async fn permute(
    ...
    for order in omg_core::math::option_orders(k, n) {
