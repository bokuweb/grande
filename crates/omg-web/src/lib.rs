//! Browser surface of `omg-core`. The inference engine lives in JS
...
use omg_core::calibration::CONTENT_FREE;
use omg_core::engine::to_answer;
use omg_core::readout::{Distribution, LABELS};
use omg_core::{Plan, Renderer, Request, Response, Usage};
...
/// when given.
fn distributions(
    branches: &[omg_core::RenderedBranch],
    rows: &str,
    baseline: Option<&str>,
    temperature: f32,
    what: &str,
...
impl WgpuEngine {
    ...
    pub async fn load(
        ...
        let w = omg_wgpu::Weights::load(config.as_bytes(), weights)
            .map_err(|e| JsError::new(&format!("{e:#}")))?;
        let inner = omg_wgpu::Engine::new(&w, capacity as usize, max_rows as usize)
            .await
            .map_err(|e| JsError::new(&format!("{e:#}")))?;
        Ok(WgpuEngine { inner })
    }
    ...
    pub async fn evaluate_rows(
        ...
        let branches: Vec<omg_core::BranchTokens> = branches
            .into_iter()
            .map(|b| omg_core::BranchTokens {
                tokens: b
                    .tokens
                    .into_iter()
                    .map(|t| omg_core::Token(t as i32))
                    .collect(),
                want: b.want,
            })
            .collect();
        let want = match want {
            "logits" => omg_core::Want::Logits,
            _ => omg_core::Want::Hidden,
        };
        let out = self
            .inner
...
pub struct WgpuLoader {
    inner: Option<omg_wgpu::EngineBuilder>,
}
...
impl WgpuLoader {
    ...
    pub async fn open(config: &str) -> Result<WgpuLoader, JsError> {
        ...
        let cfg = omg_wgpu::Config::from_json(&v).map_err(|e| JsError::new(&format!("{e:#}")))?;
        let inner = omg_wgpu::EngineBuilder::new(cfg)
            .await
            .map_err(|e| JsError::new(&format!("{e:#}")))?;
    ...
    pub fn push(
        ...
        let dtype = omg_wgpu::Dtype::parse(dtype).map_err(|e| JsError::new(&format!("{e:#}")))?;
        let t = omg_wgpu::QTensor::from_raw(
            dtype,
...
pub struct LayaEngine {
    inner: omg_wgpu::laya::LayaEngine,
    name: String,
}
...
#[cfg(feature = "wgpu")]
impl omg_wgpu::laya::prompt::Tokenize for JsTokenize<'_> {
...
#[cfg(feature = "wgpu")]
impl omg_wgpu::e5::Tokenize for JsTokenize<'_> {
    fn encode(&self, text: &str) -> Vec<u32> {
        omg_wgpu::laya::prompt::Tokenize::encode(self, text)
    }
}
...
#[wasm_bindgen]
pub struct E5Loader {
    inner: Option<omg_wgpu::e5::E5Builder>,
    name: String,
}
...
impl E5Loader {
    ...
    pub async fn open(config: &str) -> Result<E5Loader, JsError> {
        ...
        let cfg =
            omg_wgpu::e5::E5Config::from_json(&v).map_err(|e| JsError::new(&format!("{e:#}")))?;
        let name = cfg.name.clone();
        let inner = omg_wgpu::e5::E5Builder::new(cfg)
            .await
            .map_err(|e| JsError::new(&format!("{e:#}")))?;
        Ok(E5Loader {
            inner: Some(inner),
...
pub struct LayaLoader {
    inner: Option<omg_wgpu::laya::LayaBuilder>,
    name: String,
}
...
impl LayaLoader {
    ...
    pub async fn open(config: &str, specials: &str) -> Result<LayaLoader, JsError> {
        ...
        let cfg = omg_wgpu::laya::config_from_export(&v, |s| ids.get(s).copied())
            .map_err(|e| JsError::new(&format!("{e:#}")))?;
        let name = v["laya_name"].as_str().unwrap_or("laya").to_string();
        let inner = omg_wgpu::laya::LayaBuilder::new(cfg)
            .await
            .map_err(|e| JsError::new(&format!("{e:#}")))?;
        Ok(LayaLoader {
            inner: Some(inner),
