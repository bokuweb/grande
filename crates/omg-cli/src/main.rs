//! `omg`: run a System One style request against a local GGUF.
...
use omg_core::{Backend, Engine, Mode, Readout, Renderer, Request};
use omg_llama::{LlamaEngine, Options};
...
enum Cmd {
    ...
    Probe {
        ...
        #[arg(long, num_args = 0..=1, default_missing_value = omg_core::calibration::CONTENT_FREE)]
...
fn load_backend(path: &Path, opts: Options) -> Result<Box<dyn Backend + Send>> {
    if path.is_dir() {
        ...
        let mut b = omg_wgpu::WgpuBackend::load(path, n_ctx, 256)?;
...
fn readout_for(backend: &dyn Backend, head: Option<&PathBuf>) -> Result<(Renderer, Readout)> {
    match head {
        Some(p) => {
            let (h, layout) = omg_core::readout::safetensors::load_with_layout(&std::fs::read(p)?)?;
            anyhow::ensure!(
                h.d == backend.n_embd(),
...
/// question and option.
fn answer_delta(a: &omg_core::Response, b: &omg_core::Response) -> f64 {
    use omg_core::Answer;
...
fn main() -> Result<()> {
    ...
    match Cli::parse().cmd {
        Cmd::Jglue {
            ...
            use omg_eval::jglue::{self, Task};
            use omg_eval::report::{summarize, Row};
            let task = match task {
            ...
            for (i, item) in items.iter().enumerate() {
                ...
                let (_, d) = &dists[0];
                let pred = omg_core::math::argmax(&d.probs);
                ...
                if permute > 1 {
                    ...
                    for order in omg_core::math::option_orders(k, permute)
                        .into_iter()
                        .skip(1)
                    {
                        ...
                        permuted_preds.push(omg_core::math::argmax(&orig));
        ...
        Cmd::Bench {
            ...
            let mut first: Option<omg_core::Response> = None;
            let mut last: Option<omg_core::Response> = None;
        ...
        Cmd::Iia {
            ...
            let distractors = if let Some(path) = suite {
                for rec in omg_eval::suite::load(&path)? {
            ...
            for (i, (req, qid)) in items.iter().enumerate() {
                ...
                flips += usize::from(omg_core::math::argmax(p1) != a);
        ...
        Cmd::Serve {
            ...
            if omg_wgpu::E5Backend::is_e5_dir(&model) {
                let mut e5 = omg_wgpu::E5Backend::load(&model, n_ctx as usize, 256)?;
                e5.temperature = temperature;
                let name = e5.model.clone();
                let state = std::sync::Arc::new(omg_server::AppState {
                    engine: omg_server::EngineHandle::spawn(e5, max_batch),
                    api_key,
                    model_id: name.clone(),
                });
                ...
                return rt.block_on(async move {
                    ...
                    eprintln!("omg: http://{host}:{port}/v1/systemone  backend {name} (e5)  temperature x{temperature}");
                    axum::serve(listener, omg_server::router(state)).await?;
                    Ok::<(), anyhow::Error>(())
                });
            }
            // A Laya checkpoint (encoder + decision head) has its own runtime.
            if omg_wgpu::LayaBackend::is_laya_dir(&model) {
                let mut laya = omg_wgpu::LayaBackend::load(&model, n_ctx as usize, 1024)?;
                laya.temperature = temperature;
                let name = laya.model.clone();
                let state = std::sync::Arc::new(omg_server::AppState {
                    engine: omg_server::EngineHandle::spawn(laya, max_batch),
                    api_key,
                    model_id: name.clone(),
                });
                let rt = tokio::runtime::Runtime::new()?;
                return rt.block_on(async move {
                    let listener = tokio::net::TcpListener::bind((host.as_str(), port)).await?;
                    eprintln!("omg: http://{host}:{port}/v1/systemone  backend {name} (laya)  temperature x{temperature}");
                    axum::serve(listener, omg_server::router(state)).await?;
                    Ok::<(), anyhow::Error>(())
                });
            }
            ...
            engine.recheck = recheck;
            let state = std::sync::Arc::new(omg_server::AppState {
                engine: omg_server::EngineHandle::spawn(engine, max_batch),
                api_key,
                model_id: name.clone(),
            });
            let rt = tokio::runtime::Runtime::new()?;
            rt.block_on(async move {
                let listener = tokio::net::TcpListener::bind((host.as_str(), port)).await?;
                eprintln!("omg: http://{host}:{port}/v1/systemone  backend {name}  temperature {temperature}  max batch {max_batch}");
                axum::serve(listener, omg_server::router(state)).await?;
                Ok::<(), anyhow::Error>(())
            })?;
        }
        ...
        Cmd::Suite {
            ...
            use omg_eval::report::{metrics, Row};
            use std::collections::BTreeMap;
            let mut records = omg_eval::suite::load(&data)?;
            if clean_only {
            ...
            for (i, rec) in records.iter().enumerate() {
                ...
                let (dists, diag) = match res {
                    Ok(x) => x,
                    Err(omg_core::Error::Invalid { message, .. })
                ...
                for (k, (branch, d)) in dists.iter().enumerate() {
                    ...
                    let row = Row {
                        ...
                        logits: d.logits.clone(),
                        baseline: d.baseline.clone(),
                        pred: omg_core::math::argmax(&d.probs),
        ...
        Cmd::Pieces { model, ids } => {
            ...
            for id in ids {
                println!("{id:>7}  {:?}", engine.piece(omg_core::Token(id)));
            }
        }
        Cmd::Features {
            model,
            ...
            let k_max = items
                ...
                .map(|it| match it.request.questions.values().next() {
                    Some(omg_core::Question::Choice { criteria, .. }) => criteria.len(),
                    Some(omg_core::Question::Score { criteria, .. }) => criteria.len(),
                    _ => 2,
                })
                .max()
                .unwrap_or(0);
            ...
            for (i, item) in items.iter().enumerate() {
                ...
                for o in 0..orders {
                    ...
                    for ((_, mark), row) in branch.marks.iter().zip(&out.rows) {
                        match mark {
                            omg_core::render::Mark::OptEnd(_) => op.push(row),
                            omg_core::render::Mark::Decide => dec = Some(row),
                            omg_core::render::Mark::Last => {}
                        }
                    }
