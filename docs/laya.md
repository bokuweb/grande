...
from grande: a small bidirectional encoder with a trained decision head,
...
Max. This note measures the multilingual checkpoint on grande's Japanese
evals and on JevBench, says what of Laya is worth taking, and describes
its port onto grande's wgpu engine (native and browser).
...
(cost O(Q × (S + q))); grande shares the state prefix (O(S + Q × q)) and
...
`tools/laya_serve.py` wraps laya-mlx in grande's `/v1/systemone` shape, so
...
grande rows are the README's / [comparison.md](comparison.md)'s.
...
| JNLI | grande E2B zero-shot | 1,217 | 0.614 | 0.252 → 0.088 | 1.255 → 0.949 | 2.81 | 754 |
| JNLI (first 400) | laya-multilingual | 400 | 0.670 | 0.242 | 1.251 | – | 18 |
| JNLI (first 400) | grande E2B + trained pointer head | 400 | **0.848** | 0.056 | – | – | 421 |
| JCQA | laya-multilingual | 559 | 0.551 | 0.041 → 0.043 | 1.177 → 1.173 | 1.23 | **21** |
| JCQA | grande E2B zero-shot | 559 | **0.853** | 0.044 → 0.046 | 0.447 → 0.438 | 1.19 | 702 |
| JCQA (first 400) | grande E4B zero-shot | 400 | **0.932** | – | – | – | – |
...
scaling (22 % of its ≥0.9 answers wrong). grande's trained head on the
...
|---|---|---|---|---|---|
| grande, Gemma 4 E4B it Q4_0, zero-shot | 100.0 | 94.4 | 51.4 | 77.6 (#10) | 1.47 s / 17.5 s |
| grande, Gemma 4 E2B it Q4_0 pruned, zero-shot | 97.9 | 73.6 | 34.2 | 61.9 (#17) | – |
...
K 59) against 59 for grande E4B — the speed term cannot carry it.

## What to take from Laya

1. **Temperature per (question type, option count).** Laya's
   `temperature[3]` + `temperature_by_options{"choice:5": …}`; grande has
   ...
   JevBench's calibration term. → `grande-core::calibration` bucket fit,
   `serve --temperature-file`.
2. **An "escalate" signal.** Laya trains `act_head` on (top-p, margin,
   entropy, k) against a cost of being wrong. grande measures the
   confident-error rate but does not expose anything per answer; a
   rule on the same features in the diagnostics / `X-Grande-*` headers is
   most of the value without training.
3. **A no-growth test.** laya-mlx checks 100 repeated calls for zero
   active-memory growth; `grande-wgpu` should have the same after the
...
Not worth taking: shape specialization / `pad_to_multiple` (grande-wgpu
...
grande's two-stage Choice handles 77 options), fp16 unquantized weights
(bandwidth-bound; Q4 is faster on the same GPU).

## Laya on grande's wgpu engine

Done (this branch): `crates/grande-wgpu/src/laya.rs` runs the mmBERT /
...
- `Decider` (grande-core) is the trait one level above `Backend`
  (`answer`, `answer_many`, `distributions`); `Engine<B>` and `LayaBackend`
  both implement it, and `grande serve` / `grande probe` pick the Laya
...
```bash
# grande's own runtime (wgpu), from the Hub snapshot laya-mlx downloads
hf download aac6fef/laya-multilingual-mlx
./target/release/grande serve --model ~/.cache/huggingface/hub/models--aac6fef--laya-multilingual-mlx/snapshots/<rev> --port 8791
