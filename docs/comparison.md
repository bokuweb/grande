# omg vs kev vs reflex (vs Jev where numbers exist)

2026-09-19. Machine for every "here" number: Apple M4 (base, 10-core GPU), 16 GB,
macOS. omg = llama.cpp Metal; kev = PyTorch fp32 MPS; reflex browser =
transformers.js WebGPU in the same Chromium pane as omg's browser demo.

Three things are being compared and they should not be collapsed into one
score: **accuracy on Japanese** (what omg is for), **accuracy on kev's
English suite** (the only place all three of omg, kev and Jev were scored
...
|---|---|---|---|---|---|---|
| **omg** Gemma 4 E2B it Q4_0, zero-shot label readout | – | **0.614** (n=1,217) | 0.252 → 0.088 (T=2.81) | **0.853** (n=559) | 0.044 → 0.046 | 700–750 |
| omg Gemma 4 E2B, **vocab-pruned** (1.27 GB) | – | 0.580 (first 400; unpruned 0.575 on the same rows) | 0.279 | 0.855 (unpruned 0.855) | 0.046 | 570–840 |
| omg Gemma 3 270M + LoRA + pointer head | 1,500 JNLI + 1,500 JCQA train, 2 ep | 0.540 (n=200) | 0.112 → 0.091 | 0.670 (n=200) | 0.074 | 47–77 |
| omg 270M + head, **12k records** (6,000 + 6,000, 1 ep) | 12k | **0.710** (n=200) | 0.160 → 0.086 | 0.710 (n=200) | 0.050 | 73–77 |
| omg 270M **12 of 18 layers** + head, 6k records, vocab-pruned (**256 MB**) | 6k | 0.685 (n=200) | 0.063 | 0.630 (n=200) | 0.064 | **26–34** |
...
kev's published `docs/kev-vs-jev-summary.json`. omg here is the zero-shot
E2B label readout with no English tuning at all.

| task | n | kev-0.5b (trained on these sources) | Jev | omg E2B zero-shot |
|---|---|---|---|---|
...
omg ECE on this suite: 0.197 raw (no temperature), NLL 1.337 (banking77's
...
| task | omg E2B zero-shot | kev-0.5b (v1) | Jev (v1) |
|---|---|---|---|
...
All omg native numbers below were re-measured with the GPU otherwise idle
...
|---|---|---|---|---|---|
| omg native (Metal) | Gemma 3 270M f16 + head | 12 Q, 500-tok state | 1,072 | **168 ms** | **86 ms** |
| omg native | Gemma 4 E2B Q4_0 | 12 Q, 500-tok state | 1,072 | 1.94 s | 1.01 s |
| omg native | Gemma 4 E2B Q4_0 | 12 Q, 2,000-tok state | 2,620 | 4.69 s | 1.11 s |
| omg native | Gemma 4 E2B Q4_0 | 5 Q ticket | 313 | ~0.6 s | |
| omg native | Gemma 3 270M + head | JGLUE, 1 Q | ~100 | 47–77 ms | |
| kev-0.5b (PyTorch MPS fp32) | Qwen2.5-0.5B | 5 Q ticket | 261 | 211–313 ms | |
| kev-0.5b | JGLUE, 1 Q | | | 136–234 ms | |
| omg browser (WebGPU, `shared`) | Gemma 3 270M q4f16 | 8 Q contract | 639 | 1.7 s | 0.9 s |
| omg browser (`shared`) | Gemma 4 E2B q4f16 | 5 Q ticket | 313 | 2.8 s | 2.5 s |
| omg browser (`shared`) | Gemma 4 E2B q4f16 | 8 Q contract | 639 | 5.2 s | 3.1 s |
| omg browser (`batched`, state re-read per row) | Gemma 4 E2B q4f16 | 5 Q ticket | 673 | 4.6 s | |
| omg browser (`batched`) | Gemma 4 E2B q4f16 | 8 Q contract | 2,263 | 12.1 s | |
| **omg browser, trained** | grande-270m-ja q8 (211 MB) | 5 Q ticket | 571 | 449 ms | **267 ms** |
| omg browser, trained, **wgpu engine** (GPU shared with a training job; ONNX path interleaved: 0.86–1.35 s) | grande-270m-ja-wgpu f16 (320 MB) | 5 Q ticket | 240 | 0.17–0.55 s | |
| omg browser, trained, wgpu engine (same conditions; ONNX 2.9–4.0 s) | grande-270m-ja-wgpu f16 | 8 Q contract | 522 | 0.41–0.74 s | |
| omg browser, trained | grande-270m-ja q8 | 8 Q contract | 2,102 | | 923 ms |
| **omg browser, wgpu engine** (one pass, no resident state) | gemma-4-e2b-wgpu Q4_0 (2.8 GB) | 5 Q ticket | 313 | 1.16 s | |
| omg browser, wgpu engine | gemma-4-e2b-wgpu Q4_0 | 8 Q contract | 639 | 2.5 s | |
| omg native, wgpu engine | gemma-4-e2b-wgpu Q4_0 | 5 Q ticket | 313 | 1.47 s | |
| omg browser, wgpu engine, **f16-tile matmul** (pruned model; same session: previous kernels 1.18–1.42 s / 2.56–2.90 s) | gemma-4-e2b-wgpu-ja Q4_0 (1.2 GB) | 5 Q ticket / 8 Q contract | 313 / 639 | **1.02 s / 2.3 s** | |
| omg native, wgpu engine, f16-tile matmul (previous kernels 1.53 s) | gemma-4-e2b-wgpu-ja Q4_0 | 5 Q ticket | 313 | **0.88 s** (1.12 s with `OMG_WGPU_CHECKED=1`, naga's bounds clamps on) | |
| omg native, wgpu engine, + attention rewrite and 32-row matmul tiles | gemma-4-e2b-wgpu-ja Q4_0 | 5 Q ticket | 313 | **0.82 s** (GPU 782 ms: gate_up 393, down 202, qkv 55, o 51, attention 46, rest 35) | |
| omg browser, wgpu engine, **E4B** (2 K/V heads; machine shared with a llama.cpp build, so an upper bound) | gemma-4-e4b-wgpu-ja Q4_0 (2.5 GB) | 5 Q ticket / 8 Q contract | 313 / 674 | 2.5 s / 7–10 s | |
| omg native, wgpu engine, E4B (llama.cpp 2.2 s on the same GGUF, same conditions) | gemma-4-e4b-wgpu-ja Q4_0 | 5 Q ticket | 313 | 1.8 s | |
| omg native, wgpu engine, **resident state** | gemma-4-e2b-wgpu-ja Q4_0 | 12 Q, 500-tok state | 1,123 | 3.02 s | **1.66 s** |
| omg native, wgpu engine, resident state | gemma-4-e2b-wgpu-ja Q4_0 | 12 Q, 2,000-tok state | 2,663 | 8.1 s | **2.03 s** |
| omg browser, wgpu engine, resident state | gemma-4-e2b-wgpu-ja Q4_0 | 5 Q ticket / 8 Q contract | 313 / 674 | 1.0 s / 2.3 s | **0.77 s / 1.5 s** |
| omg native, wgpu engine, **state cache** (2,000-tok state seen before, another state in between) | gemma-4-e2b-wgpu-ja Q4_0 | 12 Q, 2,000-tok state | 2,663 | 8.2 s | RAM **2.05 s**, file 2.08 s (save 23 ms, 19 MB) |
| omg browser, wgpu engine, state cache | gemma-4-e2b-wgpu-ja Q4_0 | 8 Q contract, after a ticket request | 674 | | restored 1.50–1.75 s (resident 1.5–1.73) |
...
f16 **6,181 tok/s**. omg's packed pass: 553 and 6,380 tok/s. The runtime
...
questions (queue=account 62%, refund 71%); omg E2B got all 5.
...
| wgpu native: skip naga's per-index bounds clamps (`create_shader_module_trusted`; `OMG_WGPU_CHECKED=1` puts them back) | 1.12 → 0.88 s, same answers; the browser cannot (Tint's clamps are not optional) | done, default |
...
| wgpu attention: the per-tile "can any row see this tile" test over KB x ROWS (query, key) pairs by one invocation replaced by KB x runs tests (a workgroup's tokens are consecutive rows: a few runs of one sequence with contiguous positions); residual-stream RMSNorms carry the norm that reads the stream next (`rmsnorm.wgsl` `second`: post-attention + pre-feedforward, per-layer-chain norm + next layer's input norm / final norm) | E2B ticket 495 → 425 dispatches, answers bitwise equal; attention and norm time within the noise of a loaded machine (attention 62 / 61 / 69 → 60 / 74 / 61 ms, norm 11.6 / 10.5 / 14.3 → 9.1 / 15.7 / 9.7 ms over 35 layers; wall 1.31–1.37 → 1.30–1.38 s at load 29) — at most ~1% of the pass either way, since 90% of it is the matmuls | done, neutral |
