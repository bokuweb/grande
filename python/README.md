...
inference time; the outputs are `head.safetensors` (read by `grande-core`)
and a merged GGUF.

```
render.py   layout, byte-for-byte the same as grande-core::render; parity check against `grande render`
...
Parity check (run after `grande render` dumps a request):

```python
from transformers import AutoTokenizer
from grande_train.render import Renderer, check_parity
import json
...
grande probe --model grande-e2b.gguf --head runs/grande-e2b/head.safetensors --request examples/ticket-ja.json
```
