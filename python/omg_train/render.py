"""Packed branch layout, byte-for-byte the same as `omg-core::render`.
...
side is the reference. `omg render --request r.json` dumps its token ids
...
def state_text(v) -> str:
    ...
    `omg-core::api::state_text`; strings pass through."""
...
def neutralize(text: str) -> str:
    """Mirror of omg-llama's `neutralize_specials`: break `<name` so caller
...
def check_parity(enc: Encoded, dump: dict) -> list[str]:
    """Compare against `omg render` output. Returns a list of mismatches."""
    problems = []
    n = len(dump["prefix"])
