 ...
 * @returns {string}
 */
export function answer(request, rows, temperature, model, input_tokens) {
    let deferred5_0;
    let deferred5_1;
    try {
        ...
        const ret = wasm.answer(ptr0, len0, ptr1, len1, temperature, ptr2, len2, input_tokens);
        var ptr4 = ret[0];
        var len4 = ret[1];
        if (ret[3]) {
            ptr4 = 0; len4 = 0;
 ...
 * @returns {string}
 */
export function render(request, layout) {
    let deferred4_0;
    let deferred4_1;
    try {
        const ptr0 = passStringToWasm0(request, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('grande_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
