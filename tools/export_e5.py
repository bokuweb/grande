"""Package multilingual-e5-small (a BERT encoder) plus the (state, option)
head of tools/e5_generic.py for grande's wgpu engine
(crates/grande-wgpu/src/e5.rs), natively and in the browser.

Weights go to the engine's manifest layout (model::Manifest): f16, or Q8_0
codes with f16 block scales (`--dtype q8`). Q, K, V are fused into one
`[3d, d]` projection with 1/sqrt(head_dim) folded into Q (the attention
kernel has no scale), token type row 0 becomes the embedding bias, and the
head's feature z-scoring (mu, sd) is folded into its first Linear. With
...
def prune(tok_json: dict, hf, texts: list[str], top: int) -> tuple[dict, np.ndarray]:
    ...
    out["model"]["unk_id"] = new_id[model.get("unk_id", 3)]
...
def main():
    ...
    ap.add_argument("--temperature", type=float, default=1.4, help="the head's calibration temperature (tools/e5_serve.py's default)")
    ...
    embed = W["embeddings.word_embeddings.weight"]
    if a.corpus:
    ...
    g = {"name": a.name or out.name, "cls": ids["<s>"], "sep": ids["</s>"], "pad": ids["<pad>"], "prefix": "query: ",
         "max_len": cfg["max_position_embeddings"], "head_in": 4 * d, "head_hidden": int(l1.shape[0]), "temperature": a.temperature,
